...
def _train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        # 打印所有可训练参数的详细信息
        # logging.info("\n=== Trainable Parameters Detail ===")
        # total_trainable = 0
        # for name, param in model._network.named_parameters():
        #     if param.requires_grad:
        #         param_size = param.numel()
        #         total_trainable += param_size
        #         logging.info(f"{name}: shape={list(param.shape)}, size={param_size:,}")
        # logging.info(f"Total trainable parameters confirmed: {total_trainable:,}\n")
